# `cct_inverse_kinematics` — Implementation Plan
...
`cct_inverse_kinematics` is a **robot-agnostic inverse-kinematics service**. Given a
...
2. **Virtual links / tool frames.** Allow attaching a 6-DOF offset to an existing
   ...
   simulate the robot holding a tool. Reuse `cct_common.urdf_loader.augment_urdf`.
...
python_3rdlib/cartesian_controllers_toolkit/
  cct_inverse_kinematics/
    ...
    resource/cct_inverse_kinematics
    test/
...
- Build: `colcon build --symlink-install --packages-select cct_inverse_kinematics`.

### `setup.py` entry points (mirror `robot_feasibility_test`)

```python
entry_points={'console_scripts': [
    'ik_node = cct_inverse_kinematics.ik_node:main',
    'dashboard_node = cct_inverse_kinematics.dashboard_node:main',
    'cct_ik = cct_inverse_kinematics.cli:main',
]}
...
`std_srvs`, `tf2_ros`, `cct_common`, `python3-numpy`. Pinocchio is a system/pip
...
- Two mechanisms, both reusing `cct_common.urdf_loader`:
...
- **Phase B (recommended for robust integration): a tiny typed interfaces package
  `cct_ik_interfaces` (`ament_cmake`)** with `SolveIK.srv` and `IKTask.msg`
...
via `cct_common.urdf_loader`. Unit tests vs known FK (and optionally PyKDL
cross-check).
...
`README.md`; finalize `test/`. (Optional) **Phase 6**: typed `cct_ik_interfaces`
...
- **Pure-`ament_python`, JSON API for the MVP**; typed `cct_ik_interfaces` is a
