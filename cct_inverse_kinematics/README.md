...
| R2 | **Virtual tool frames** — attach a 6-DOF offset to a link and solve on it (via `cct_common.urdf_loader.augment_urdf`) |
...
```bash
colcon build --symlink-install --packages-select cct_inverse_kinematics
...
cct_ik validate --urdf /tmp/r.urdf --frame right_arm_Link7 --n 200   # round-trip gate
cct_ik fk       --urdf /tmp/r.urdf --frame right_arm_Link7
cct_ik solve    --urdf /tmp/r.urdf --frame right_arm_Link7 --xyz 0.4 -0.2 0.9
```
...
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python3 -m pytest \
  python_3rdlib/cartesian_controllers_toolkit/cct_inverse_kinematics/test -q
```
...
continuity, E4 RViz interactive marker, a typed `cct_ik_interfaces` service
