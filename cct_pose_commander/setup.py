...
package_name = 'cct_pose_commander'

setup(
    name=package_name,
    version='0.1.0',
    packages=find_packages(exclude=['test']),
    # Ship the dashboard's static web assets (HTML/CSS/JS) inside the package so
    ...
    description='Accepts a Cartesian target pose on a topic, solves it with the '
                'cct_inverse_kinematics solver, and commands the arm via its '
    ...
    entry_points={
        'console_scripts': [
            'commander_node = cct_pose_commander.commander_node:main',
            'dashboard_node = cct_pose_commander.dashboard_node:main',
            'send_pose = cct_pose_commander.send_pose:main',
        ],
    },
)
