#!/usr/bin/env python3
"""cct_pose_commander — accept a Cartesian target pose, solve IK, move the arm.

Pipeline per target (one configured kinematic group = frame + joints + a
controller pair):

    PoseStamped  ->  (TF to base)  ->  cct_inverse_kinematics solve
                 ->  SAFETY GATE   ->  command the arm

This node ACTUALLY commands the robot (unlike cct_inverse_kinematics, which is
...
try:
    from cct_inverse_kinematics.robot_model import RobotModel
    from cct_inverse_kinematics import ik_core
    from cct_inverse_kinematics.tasks import Task
...
class PoseCommander(Node):
    def __init__(self) -> None:
        super().__init__("cct_pose_commander")
        ...
        if _IK_IMPORT_ERROR is not None:
            self.get_logger().error(
                "cct_inverse_kinematics import failed: %s — the commander "
