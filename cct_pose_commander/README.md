...
[`cct_inverse_kinematics`](../cct_inverse_kinematics/README.md) solver
(in-process), and **command the arm** to that pose.

Unlike `cct_inverse_kinematics` (which is *advisory only* — it just publishes IK
results), this package **actually moves the robot**. It is therefore
**safety-gated**.

```
PoseStamped ──▶ (TF→base) ──▶ cct_inverse_kinematics solve ──▶ SAFETY GATE ──▶ command
...
colcon build --packages-select cct_inverse_kinematics cct_pose_commander --symlink-install
...
```bash
ros2 service call /cct_pose_commander_right/disable std_srvs/srv/Trigger
```
...
ros2 launch cct_pose_commander dashboard.launch.py \
    commander_ns:=/cct_pose_commander_right base_frame:=base_link port:=8180
...
* **`cct_inverse_kinematics`** — the advisory solver this node calls in-process
...
* **`cartesian_control_manager` / FZI `cartesian_motion_controller`** — an
  ...
  (with the 7-DOF null-space drift). Use `cct_pose_commander` when you want *your*
...
* Not yet: streaming continuity (velocity/jerk-bounded successive solutions);
  dual-arm relative-pose commanding (blocked on the `cct_inverse_kinematics` R9
